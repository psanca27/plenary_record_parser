...
df = df[['date', 'executive', 'ministerium', 'party', 'president', 'seq', 'servant', 'session', 'speaker', 'speech', 'state', 'sub', 'wp', 'role', 'issue']]
df['deputy'] = df['role']=='mp'
df.rename(columns={'executive': 'minister'}, inplace=True)
df.loc[df['role'] == 'secretary','minister'] = True
df['speech'] = df[['date', 'seq', 'speaker', 'speech']].groupby(['date', 'seq', 'speaker'])['speech'].transform(lambda x: ' '.join(x).strip())
...
df = df[['date', 'seq', 'speaker', 'minister', 'ministerium', 'party', 'president', 'session', 'speech', 'state', 'wp', 'deputy', 'issue']] 
