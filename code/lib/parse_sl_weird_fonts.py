...
def find_files_without_poi(filename):
#files_corrupted = []
#for filename in files:
    ...
    if '<poi_begin>' in text:
        continue
    else:
        file = str(filename.replace('_xml.txt', '.xml'))
        print(wp, session, 'no <poi>, porcoddio!')
    ...
    # gets page_id
        ...
        for textbox in textboxes:
            ...
            for line, has_more in lookahead(lines):
                ...
                for char in chars:
                    if char.attrib:
                        if char.attrib['font'] in list_fonts:
                            continue
                        else:
                            #print(char.attrib['font'], page_id, i)
                            if counter == 0:
                                list_fonts.append(char.attrib['font'])
                                counter +=1

list_fonts.append('Bold')

bold_fonts = ['EIBKOD+AdvPS_POB', 'AOOPBF+AdvPS_POB', 'AOOPEI+AdvPOB']                 
    ...
    for page in pages:
        ...
        for textbox in textboxes:
            ...
            if textbox_bounds[1]>params['header_bound'] and page_id not in ['1']:
                #import pdb; pdb.set_trace()
                print('removed header ' + textbox_text, page_id)
                continue
            ...
            if page_set=='first':
                ...
                elif textbox['left'] > params["text_margin_first_right"] + 11:
                    textbox['text'] = '<interjection_begin>' + textbox['text'].replace('\n', '<interjection_end>\n<interjection_begin>') + '<interjection_end>'
...
for filename in sorted(files_corrupted):
    print(filename)
