    ...
    for page in pages:
        ...
        for e in left_margin:
            if (e in range(params["text_margin_first_left"] - 5, params["text_margin_first_left"] + 5) or
                ...
                cnt_set_one += 1
            if (e in range(params["text_margin_second_left"] - 5, params["text_margin_second_left"] + 5) or
                e in range(params["text_margin_second_right"] - 5, params["text_margin_second_right"] + 5) or
                e in range(params["indentation_bound_second_left"] - 5, params["indentation_bound_second_left"] + 5) or
                e in range(params["indentation_bound_second_right"] - 5, params["indentation_bound_second_right"] + 5)):
                cnt_set_two += 1
...
def iteratesFiles(state):
    ...
    with open(os.path.join(DATA_PATH, "params_" + state + "_2.json"), encoding="utf-8") as fp:
