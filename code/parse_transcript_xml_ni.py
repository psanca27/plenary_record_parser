...
# interjection: 79
# text right x0: 306
# interjection: 334-335

# second set of pages:
# text left x0: 62
# interjection left: 91
# text right x0: 317-318
# interjection right: 346
    ...
    for page in pages:
        ...
        for e in left_margin:
            if (e in range(params["text_margin_first_left"] - 4, params["text_margin_first_left"] + 4) or
                ...
                cnt_set_two += 1
            elif (e in range(params["text_margin_third_left"] - 4, params["text_margin_third_left"] + 4) or
                e in range(params["text_margin_third_right"] - 4, params["text_margin_third_right"] + 4) or
                e in range(params["indentation_bound_third_left"] - 4, params["indentation_bound_third_left"] + 4) or
                e in range(params["indentation_bound_third_right"] - 4, params["indentation_bound_third_right"] + 4)):
                cnt_set_three += 1

        if cnt_set_one==0 and cnt_set_two==0 and cnt_set_three==0:
            logging.warning('no x0 values within specified ranges' + page.attrib['id'])
            page_set = None
        else:
            if cnt_set_one > cnt_set_two and cnt_set_one > cnt_set_three:
                page_set = 'first'
            elif cnt_set_two > cnt_set_one and cnt_set_two > cnt_set_three:
                page_set = 'second'
            else:
                page_set = 'third'
        ...
        for textbox in textboxes:
            ...
            if page_set=='first':
                ...
                if textbox['left'] > params["indentation_bound_third_left"] - 5 and textbox['left'] < params["text_margin_third_right"] - 5:
                    textbox['text'] = '<interjection_begin>' + textbox['text'] + '<interjection_end>'
                elif textbox['left'] > params["indentation_bound_third_right"] - 5:
                    textbox['text'] = '<interjection_begin>' + textbox['text'] + '<interjection_end>'

                if textbox['left'] < params['text_margin_third_right'] - 5:
                    textbox['left'] = 30
                else:
                    textbox['left'] = 30
                    textbox['top'] = textbox['top']-1000
