...
BEGIN_MARK = re.compile(r'^(?:Beginn der Sitzung|Beginn der Feierstunde|Beginn(?:en)?):?\s(?:[0-9]{1}|[0-9]{2})(?:.[0-9]{2})?\s?Uhr')
END_MARK = re.compile(r'^(?:Schluss\s+der\s+Sitzung|Schluss\s+der\s+Sitzung|Schluss\s?):\s+(?:[0-9]{2}|[0-9]{1})(.[0-9]{2})?(?:\s+Uhr)?')
CHAIR_MARK = re.compile(r'^<poi_begin>(Präsident(?:in)?|Alterspräsident(?:in)?|Vizepräsident(?:in)?)\s(.+?)<poi_end>')
SPEAKER_MARK = re.compile(r'^<poi_begin>(.+)(?:(?:<poi_end>?\s+|\s+<poi_end>)(?:.+)?\((SPD|CDU|FDP|GRÜNE|LINKE|fraktionslos|AfD)\)|\s+\((.+)\):?(?:\s+)?<poi_end>)')
SPEAKER_MARK_SECOND_LINE = re.compile(r'^<poi_begin>(.+)(?:(?:<poi_end>?\s+|\s+<poi_end>)(\(GRÜ-))')
EXECUTIVE_MARK = re.compile(r'^<poi_begin>(.+?)<poi_end>,?\s*((?:[Mm]inister(?:in)?|Ministerpräsident|Finanzminister(?:in)?|Kultusminister(?:in)?|Umweltminister(?:in)?|Kultusminister(?:in)?|Justizminister(?:in)?))')
...
DATE_CAPTURE = re.compile(r'^(?:(?<!Ausgegeben).)*(Hannover,)?(\s+den)?\s+([0-9]{1,2}\.\s+.+?\s+[0-9]{4})')
...
for filename in files:
    ...
    for line, has_more in helper.lookahead(lines):
        if not date_captured and DATE_CAPTURE.search(line):
            date = DATE_CAPTURE.search(line).group(3)
        ...
        if '<poi_begin>' in line:   
            if CHAIR_MARK.match(line):
                ...
                new_speaker = re.sub(' +', ' ', s.group(0)).replace(':', '').replace('<poi_begin>', '').replace('<poi_end>', '').strip()
                president = True
                executive = False
                servant = False
                party = None
                ...
                role = 'mp'
                ministerium = None
            elif SPEAKER_MARK_SECOND_LINE.match(line):
                s = SPEAKER_MARK_SECOND_LINE.match(line)
                new_speaker = re.sub(' +', ' ', s.group(1)).strip()
                president = False
                executive = False
                servant = False
                ...
                new_speaker = re.sub(' +', ' ', s.group(1)).replace('<poi_begin>', '').replace('<poi_end>', '').replace(',','').strip()
                role = 'executive'
                party = None
                president = False
                executive = True
                servant = False
                ministerium = re.sub(' +', ' ', s.group(2)).strip()
