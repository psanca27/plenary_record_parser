...
SPEECH_ENDS = re.compile("|".join([CHAIR_STRING, SPEAKER_STRING, EXECUTIVE_STRING]))
INTERJECTION_MARK = re.compile(r'^<interjection_begin>\(')
INTERJECTION_END = re.compile(r'(?:\)$|\)s+)')
...
DATE_CAPTURE = re.compile(r'([0-9]{1,2}\.\s+\w+\s+[0-9]{4}|[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{4})')
...
for filename in files:
    ...
    wp, session = int(filename[18:20]), int(filename[21:23])
    ...
    for line, has_more in helper.lookahead(lines):
        ...
        if poi:
            if POI_ONE_LINER.match(line):
                ...
                issue = issue.replace('<poi_begin>', '').replace('<interjection_begin>', '')
                issue = issue.replace('<poi_end>', '').replace('<interjection_end>', '')
                poi = False
                # poi_prev = True
        ...
        if '<poi_begin>' in line:
            ...
            if new_speaker:
                new_speaker = new_speaker.replace('Frau', '').replace(':', '').replace('<poi_end>', '').strip()
...
#text length
idx_txt = [i for i, e in enumerate(ls_text_length) if e[0] > 300]
