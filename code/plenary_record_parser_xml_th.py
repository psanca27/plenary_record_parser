...
BEGIN_STRING = r'^[Dd]ie\s+(Fest)?[Ss]itzung\s+(wird|beginnt)\s+um\s+\d\d?.?\d?\d?\s+Uhr'
...
EXECUTIVE_STRING = r'^<poi_begin>(.+?),\s+((?:Stellvertreter\s+des\s+)?Ministerpräsident(?:[ei]n)?:|Minister(?:in)?\s+für\s+.+(?::)?|Justizminister(?:in)?:|Finanzminister(?:in)?:|Kultusminister(?:in)?:|Innenminister(?:in)?:)'
...
DATE_CAPTURE = re.compile(r'((Montag|Dienstag|Mittwoch|Donnerstag|Freitag|Samstag|Sonntag),\s+den\s+)(\d\d?\.\s*(\w+|\d\d?.?)\s*\d\d\d\d)')
        ...
        if '<poi_begin>' in line:
            ...
            if new_speaker:
                ...
                if party:
                    ...
                    if 'NKE' in party:
                        party = 'DIE LINKE'
                    elif 'Linkspartei' in party:
                        party = 'Die Linkspartei.PDS'
